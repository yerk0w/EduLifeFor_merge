...
const DocumentsList = ({ selectedDocument, setSelectedDocument, setDocumentTab }) => {
  ...
  const documents = [
    {
      ...
      date: '15.04.2025',
      status: 'approved',
      statusText: t('documentsList.status.approved'),
    ...
    },
    {
      ...
      date: '10.04.2025',
      status: 'approved',
      statusText: t('documentsList.status.issued'),
    ...
    },
    {
      ...
      date: '05.04.2025',
      status: 'pending',
      statusText: t('documentsList.status.pending'),
      recipient: null
    },
    {
      id: 4, 
      ...
      date: '01.04.2025',
      status: 'draft',
      statusText: t('documentsList.status.draft'),
      recipient: null
    }
  ];
  ...
  return (
    <div className="documents-list">
      <h4 className="subsection-title">{t('documentsList.title')}</h4>
      ...
      <div className="documents-controls">
        <div className="search-container">
          ...
          <input 
            ...
            placeholder={t('documentsList.search')} 
            value={searchQuery}
        ...
        <div className="filter-container">
          ...
          >
            <option value="all">{t('documentsList.filter.all')}</option>
            <option value="договор">{t('documentsList.filter.contracts')}</option>
            <option value="сертификат">{t('documentsList.filter.certificates')}</option>
            <option value="справка">{t('documentsList.filter.references')}</option>
          </select>
        </div>
      ...
      <div className="documents-table-container">
        <table className="documents-table">
          <thead>
            <tr>
              <th>{t('documentsList.columns.name')}</th>
              <th>{t('documentsList.columns.type')}</th>
              <th>{t('documentsList.columns.date')}</th>
              <th>{t('documentsList.columns.status')}</th>
              <th>{t('documentsList.columns.recipient')}</th>
              <th>{t('documentsList.columns.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {filteredDocuments.map(doc => (
              ...
              >
                ...
                <td>{doc.recipient || t('documentsList.noRecipient')}</td>
...
const DocumentDetails = ({ documentId, selectedStudent, setSelectedStudent }) => {
  ...
  return (
    <div className="document-details">
      <h4 className="detail-title">{t('documentsList.details.title')}</h4>
      <div className="detail-content">
        <div className="detail-row">
          <div className="detail-label">{t('documentsList.details.name')}</div>
          <div className="detail-value">{document.name}</div>
        </div>
        <div className="detail-row">
          <div className="detail-label">{t('documentsList.details.type')}</div>
          <div className="detail-value">{document.type}</div>
        </div>
        <div className="detail-row">
          <div className="detail-label">{t('documentsList.details.date')}</div>
          <div className="detail-value">{document.date}</div>
        </div>
        <div className="detail-row">
          <div className="detail-label">{t('documentsList.details.status')}</div>
          <div className="detail-value">
            <span className={`status-badge ${document.status}`}>{document.statusText}</span>
          </div>
        </div>
        <div className="detail-row">
          <div className="detail-label">{t('documentsList.details.recipient')}</div>
          <div className="detail-value recipient-search">
            <div className="search-container compact">
              ...
              <input 
                ...
                placeholder={t('documentsList.details.recipientSearch')} 
        ...
        {needsRecipient && (
          <div className="document-actions">
            <div className="send-document-form">
              <h5 className="form-subtitle">{t('documentsList.sendForm.title')}</h5>
              <div className="form-group">
                <label className="form-label">{t('documentsList.sendForm.selectStudent')}</label>
                <select 
                  className="form-select" 
                  value={selectedStudent} 
                  onChange={(e) => setSelectedStudent(e.target.value)}
                >
                  <option value="">{t('documentsList.sendForm.selectStudentOption')}</option>
              ...
              <div className="form-group">
                <label className="form-label">{t('documentsList.sendForm.comment')}</label>
                <textarea 
                  className="form-textarea" 
                  rows="3" 
                  placeholder={t('documentsList.sendForm.commentPlaceholder')}
                ></textarea>
              </div>
        ...
        {(document.status === 'approved') && (
          <div className="document-actions">
            <button className="action-button download">
              ...
              {t('documentsList.actions.download')}
            </button>
            <button className="action-button send-again">
              <svg viewBox="0 0 24 24" width="20" height="20">
                <path d="M2,21L23,12L2,3V10L17,12L2,14V21Z" />
              </svg>
              {t('documentsList.actions.sendAgain')}
            </button>
        ...
        {document.status === 'draft' && (
          <div className="document-actions">
            <button className="action-button edit">
              ...
              {t('documentsList.actions.edit')}
            </button>
            <button className="action-button delete">
              <svg viewBox="0 0 24 24" width="20" height="20">
                <path d="M19,4H15.5L14.5,3H9.5L8.5,4H5V6H19M6,19A2,2 0 0,0 8,21H16A2,2 0 0,0 18,19V7H6V19Z" />
              </svg>
              {t('documentsList.actions.delete')}
            </button>
          </div>
        )}
      </div>
