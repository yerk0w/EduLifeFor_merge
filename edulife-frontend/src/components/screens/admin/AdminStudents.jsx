...
const AdminStudents = ({ students: initialStudents }) => {
  ...
  const handleDeleteStudent = async (studentId) => {
    if (window.confirm(t('adminStudents.deleteConfirm'))) {
      try {
        ...
        alert(t('adminStudents.deleteError'));
      }
    }
  };
  ...
  if (loading) {
    return (
      <div className="admin-students loading">
        <h3 className="section-title">{t('adminStudents.title')}</h3>
        <div className="loading-spinner">
          <div className="spinner"></div>
          <p>{t('adminStudents.loading')}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="admin-students">
      ...
      <div className="students-controls">
        ...
        <div className="filter-container">
          ...
          >
            <option value="all">{t('adminStudents.filter.all')}</option>
            <option value="active">{t('adminStudents.filter.active')}</option>
            <option value="inactive">{t('adminStudents.filter.inactive')}</option>
          </select>
        </div>
      </div>
      ...
      <div className="students-table-container">
        <table className="students-table">
          <thead>
            <tr>
              <th>{t('adminStudents.columns.name')}</th>
              <th>{t('adminStudents.columns.email')}</th>
              <th>{t('adminStudents.columns.group')}</th>
              <th>{t('adminStudents.columns.progress')}</th>
              <th>{t('adminStudents.columns.lastActivity')}</th>
              <th>{t('adminStudents.columns.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {currentStudents.length > 0 ? (
              currentStudents.map(student => (
                <tr key={student.id}>
                  ...
                  <td>{student.email}</td>
                  <td>{student.group_name || t('adminStudents.noGroup')}</td>
                  <td>
                  ...
                  <td>{student.last_activity || t('adminStudents.notActive')}</td>
                  <td className="actions-cell">
                    <button 
                      className="table-action-button" 
                      onClick={() => handleViewStudent(student.id)}
                      title={t('adminStudents.actions.view')}
                    >
                    ...
                    <button 
                      ...
                      title={t('adminStudents.actions.edit')}
                    >
                      <svg viewBox="0 0 24 24" width="18" height="18">
                    ...
                    <button 
                      ...
                      title={t('adminStudents.actions.delete')}
                    >
                      <svg viewBox="0 0 24 24" width="18" height="18">
