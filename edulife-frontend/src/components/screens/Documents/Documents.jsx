...
const Documents = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.title || !formData.content || !formData.file) {
      alert(t('documents.fillAllFields'));
      return;
    }
  ...
  return (
    <div className="documents-screen">
      <div className="documents-header">
        ...
        <h1 className="header-title">{t('documents.documentFlow')}</h1>
      ...
      <div className="documents-content">
        ...
        <section className="templates-section">
          <h2 className="section-title">{t('documents.templateDocuments')}</h2>
          {isLoadingTemplates ? (
            <div style={{ textAlign: 'center', padding: '20px' }}>
              <p>{t('documents.loadingTemplates')}</p>
            </div>
          ) : (
        ...
        <section className="document-submission-section">
          <h2 className="section-title">{t('documents.sendDocument')}</h2>
          <form className="document-form" onSubmit={handleSubmit}>
            <div className="form-group">
              <label htmlFor="title">{t('documents.documentName')}</label>
              <input 
                type="text" 
                id="title" 
                name="title" 
            ...
            <div className="form-group">
              <label htmlFor="content">{t('documents.description')}</label>
              <input 
                type="text" 
                id="content" 
                name="content" 
            ...
            <div className="form-group">
              <label htmlFor="template_type">{t('documents.documentType')}</label>
              <select 
                id="template_type" 
                name="template_type" 
            ...
            <div className="form-group file-upload-group">
              <label className="file-label">{t('documents.uploadDocument')}</label>
            ...
            >
              {isSubmitting ? t('documents.sendingDocument') : submitSuccess ? t('documents.documentSent') : t('documents.sendDocumentButton')}
            </button>
          </form>
        </section>
        ...
        <section className="user-documents-section" style={{ marginTop: '40px' }}>
          <h2 className="section-title">{t('documents.yourDocuments')}</h2>
          {isLoadingDocuments ? (
            <div style={{ textAlign: 'center', padding: '20px' }}>
              ...
              {userDocuments.map(document => (
                <div key={document.id} className="document-item" style={{ 
                  ...
                  <div style={{ fontSize: '12px', color: '#999', display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <span>{t('documents.creationDate', { date: new Date(document.created_at).toLocaleDateString() })}</span>
                    {document.template_type && <span>{t('documents.docType', { type: document.template_type })}</span>}
                    {document.author_name && document.author_name !== localStorage.getItem('username') && 
                      <span>{t('documents.from', { author: document.author_name })}</span>
                    }
                  </div>
        ...
        {userRole !== 'admin' && userRole !== 'админ' && (
          <section className="admin-documents-section" style={{ marginTop: '40px' }}>
            <h2 className="section-title">{t('documents.adminDocuments')}</h2>
            {isLoadingAdminDocuments ? (
              <div style={{ textAlign: 'center', padding: '20px' }}>
                <p>{t('documents.loadingAdminDocuments')}</p>
              </div>
                ...
                {adminDocuments.map(document => (
                  <div key={document.id} className="document-item" style={{ 
                    ...
                    <div style={{ fontSize: '12px', color: '#999', display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                      <span>{t('documents.creationDate', { date: new Date(document.created_at).toLocaleDateString() })}</span>
                      {document.template_type && <span>{t('documents.docType', { type: document.template_type })}</span>}
                      {document.author_name && <span>{t('documents.from', { author: document.author_name })}</span>}
                    </div>
                    {document.file_path && (
                      <button 
                        onClick={() => handleOpenDocument(document.file_path)}
