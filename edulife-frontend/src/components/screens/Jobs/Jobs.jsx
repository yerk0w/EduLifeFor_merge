...
const Jobs = () => {
  ...
  return (
    <div className="jobs-screen">
      ...
      {/* Заголовок */}
      <h1 className="header-title">{t('jobs.vacancies')}</h1>
      ...
      {!showSearch && (
        <div className="jobs-tabs">
          <h2 className="tab-title">{t('jobs.internships')}</h2>
        </div>
      )}
        ...
        {searchQuery && (
          <div className="search-results-info">
            {searchResults.length > 0 ? (
              <p>{t('jobs.searchResults', { count: searchResults.length })}</p>
            ) : (
              <p>{t('jobs.noResults', { query: searchQuery })}</p>
            )}
          </div>
        )}
