...
const CourseDetail = ({ courses }) => {
  const { t } = useTranslation('screens'); // Укажите пространство имён 'screens'
  const { id } = useParams();
  ...
  const course = courses.find(c => c.id === id) || {
    ...
    homework: [
      {
        question: t('courseDetail.question1'),
        options: [
          t('courseDetail.option1_1'),
          t('courseDetail.option1_2'),
          t('courseDetail.option1_3'),
          t('courseDetail.option1_4'),
        ],
        correctAnswer: 1
      },
      {
        question: t('courseDetail.question2'),
        options: [
          t('courseDetail.option2_1'),
          t('courseDetail.option2_2'),
          t('courseDetail.option2_3'),
          t('courseDetail.option2_4'),
        ],
        correctAnswer: 1
      },
      {
        question: t('courseDetail.question3'),
        options: [
          t('courseDetail.option3_1'),
          t('courseDetail.option3_2'),
          t('courseDetail.option3_3'),
          t('courseDetail.option3_4'),
        ],
        correctAnswer: 2
      }
    ]
  ...
  const renderHomework = () => {
    if (!course.homework || course.homework.length === 0) {
      return (
        <div className="homework-empty">
          <p>{t('courseDetail.homeworkNotAvailable')}</p>
        </div>
      );
    }
    ...
    return (
      <div className="homework-quiz">
        <h3>{t('courseDetail.materialTest')}</h3>
        ...
        <button className="submit-homework">{t('courseDetail.submitAnswers')}</button>
      </div>
    );
  };
  ...
  return (
    <div className="course-detail-screen">
      <div className="course-detail-header">
        ...
        <h1 className="header-title">{t('courseDetail.aboutSubject')}</h1>
